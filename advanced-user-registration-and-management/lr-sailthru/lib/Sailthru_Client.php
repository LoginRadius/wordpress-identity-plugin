...
class LR_Sailthru_Client {
    ...
    private $options = array('timeout' => LR_Sailthru_Client::DEFAULT_READ_TIMEOUT, 'connect_timeout' => LR_Sailthru_Client::DEFAULT_CONNECT_TIMEOUT);
    ...
    public function  __construct($api_key, $secret, $api_uri = false, $options = null) {
        ...
        if (isset($options)) {
            $this->options['timeout']         = isset($options['timeout']) ? (int)$options['timeout'] : LR_Sailthru_Client::DEFAULT_READ_TIMEOUT;
            $this->options['connect_timeout'] = isset($options['connect_timeout']) ? (int)$options['connect_timeout'] : LR_Sailthru_Client::DEFAULT_CONNECT_TIMEOUT;
        }
    }
    ...
    public function receiveVerifyPost() {
        ...
        $sig = $params['sig'];
        unset($params['sig']);
        if ($sig != LR_Sailthru_Util::getSignatureHash($params, $this->secret)) {
            return false;
        }
    ...
    protected function httpRequestCurl($url, array $data, $method = 'POST', $options = array()) {
        ...
        curl_close($ch);
        if (!$data) {
            throw new LR_Sailthru_Client_Exception("Bad response received from $url");
        }
        return $data;
    }
    ...
    protected function httpRequestWithoutCurl($url, $data, $method = 'POST', $options = array()) {
        if ($this->fileUpload === true) {
            $this->fileUpload = false;
            throw new LR_Sailthru_Client_Exception('cURL extension is required for the request with file upload');
        }
        ...
        if (!$fp) {
            throw new LR_Sailthru_Client_Exception("Unable to open stream: $url");
        }
        $response = @stream_get_contents($fp);
        if ($response === false) {
            throw new LR_Sailthru_Client_Exception("No response received from stream: $url");
        }
        return $response;
    }
    ...
    protected function httpRequest($url, $data, $method = 'POST', $options = array()) {
        ...
        if ($json === NULL) {
            throw new LR_Sailthru_Client_Exception("Response: {$response} is not a valid JSON");
        }
        return $json;
    }
    ...
    protected function prepareJsonPayload(array $data, array $binary_data = array()) {
        ...
        $payload['sig'] = LR_Sailthru_Util::getSignatureHash($payload, $this->secret);
