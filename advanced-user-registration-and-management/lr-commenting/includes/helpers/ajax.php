...
class Ajax_Helper {
    ...
    function loginradius_post_comment() {
        global $wpdb, $loginRadiusObject, $loginRadiusSettings, $lr_commenting_settings, $loginradius_api_settings;
        ...
        if ( ! empty( $_POST['share_facebook_token'] ) ) {
             ...
             */
            try {
                $facebook_msg = $loginRadiusObject->loginradius_post_status( $accessToken, $post_title, $post_url, $image_url, $status, $site_caption, $site_description );
            } catch (LoginRadiusException $e) {
        ...
        if ( ! empty( $_POST['share_twitter_token'] ) ) {
             ...
             */
            try {
                $twitter_msg = $loginRadiusObject->loginradius_post_status( $accessToken, $post_title, $post_url, $image_url, $status, $site_caption, $site_description );
            } catch (LoginRadiusException $e) {
        ...
        if ( ! empty( $_POST['share_linkedin_token'] ) ) {
             ...
             */
            try {
                $linkedin_msg = $loginRadiusObject->loginradius_post_status( $accessToken, $post_title, $post_url, $image_url, $status, $site_caption, $site_description );
            } catch ( LoginRadiusException $e ) {
                error_log( 'Linkedin Post Status Failure ' . $e->errorResponse->providerErrorResponse );
                $linkedin_msg = false;
            } 
        }
