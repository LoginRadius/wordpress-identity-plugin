...
Class LR_CI_Ajax_Helper {
    ...
    public function __construct() {
        self::$custom_interface_dir = LR_CUSTOM_INTERFACE_DIR . 'assets/images/custom_interface';
        self::$default_interface_dir = LR_CUSTOM_INTERFACE_DIR . 'assets/images/default_interface';
    }
    ...
    private static function move_upload_files( $path ) {
        foreach ( $_FILES["images"]["error"] as $key => $error ) {
            if ( $error == UPLOAD_ERR_OK ) {
                ...
                $name = isset( $_POST["socialProvider"] )?trim( $_POST["socialProvider"] ).'.png':$_FILES["images"]["name"][$key];
            ...
            if ( is_main_site() ) {
                ...
                if ( isset( $loginradius_api_settings['multisite_config'] ) && $loginradius_api_settings['multisite_config'] == '1' ) {
                    ...
                    foreach ( $blogs as $blog ) {
                        $path = self::$custom_interface_dir.'/'.$blog['blog_id'];
                        if( ! file_exists( $path ) ) { 
                            wp_mkdir_p( $path );
                        }
        ...
        if( is_multisite() ){
            if ( is_main_site() ) {
                ...
                if ( isset( $loginradius_api_settings['multisite_config'] ) && $loginradius_api_settings['multisite_config'] == '1' ) {
                    $blogs = wp_get_sites();
                    foreach ( $blogs as $blog ) {
                        self::move_default_files( self::$custom_interface_dir . '/' . $blog['blog_id'] );
                    }
                }
            }else{
