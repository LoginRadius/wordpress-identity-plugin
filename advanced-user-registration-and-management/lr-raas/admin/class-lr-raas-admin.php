...
if ( ! class_exists( 'LR_Raas_Admin' ) ) {
	...
	class LR_Raas_Admin {
		...
		public function delete_user( $user_id ) {
			...
			if ( ! empty( $raas_uid ) ) {
				$response = raas_delete_user( $raas_uid );
			}
		}
									...
									<tr>
										<td colspan="2">
											...
											<?php
												...
												if ( empty( $uid ) ) {
													printf( '<div class="error"><p>' . __('An error has occurred on link account.', 'lr-plugin-slug' ) . '</p></div>' );
													return;
												}
												...
												if ( ! empty( $accountid ) && ! empty( $provider ) ) {
													$response = raas_unlink_account( $uid, $provider, $accountid );
													if ( isset( $response->isPosted ) && $response->isPosted === true ) {
														...
														$message = isset( $response->description ) ? $response->description : $message;
													}
													printf( '<div class="' . $type . ' lr_' . $type . '"><p>' . $message . '</p></div>' );
												} elseif ( isset( $_POST['token'] ) && ! empty( $_POST['token'] ) && is_user_logged_in() ) {
													$userProfileObject = $loginradiusObject->loginradius_get_user_profiledata( $_POST['token'] );
													if ( isset( $userProfileObject->Provider ) && isset( $userProfileObject->ID ) ) {
														$linkuser = get_users( 'meta_value=' . $userProfileObject->ID );
														if( count( $linkuser ) > 0 ){
															$message = __( 'Your account is already exist.', 'lr-plugin-slug' );
														}else{
															$response = raas_link_account( $uid, $userProfileObject->Provider, $userProfileObject->ID );
															if ( isset( $response->isPosted ) && $response->isPosted === true ) {
																...
																$message = isset( $response->description ) ? $response->description : __( 'An error has occurred', 'lr-plugin-slug' );
															}
														}
													}
													printf( '<div class="' . $type . ' lr_' . $type . '"><p>' . $message . '</p></div>' );
